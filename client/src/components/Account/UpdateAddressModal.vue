<template>
  <div>
    <input type="checkbox" id="modal_edit_address" class="modal-toggle" v-model="isModalEditOpen" />
    <div class="modal" role="dialog">
      <div class="modal-box max-w-[600px] bg-white py-6 px-8">
        <div>
          <h4 class="text-xl font-bold">Cập nhật địa chỉ</h4>
        </div>
        <form class="form">
            ...
            <div class="col-span-6">
              <div class="input-group">
                ...
                <input
                  ...
                  v-model="selectAddress.nameCustomer"
            ...
            <div class="col-span-6">
              <div class="input-group">
                ...
                <input
                  ...
                  v-model="selectAddress.phoneNumber"
                ...
                <div class="relative" ref="selectLocationForm">
                  <div class="relative">
                    <input
                      ...
                      v-model="selectAddress.fullAddress"
            ...
            <div class="col-span-12">
              <div class="input-group">
                ...
                <textarea
                  ...
                  v-model="selectAddress.specificAddress"
          ...
          <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12">
              <div class="relative mt-4 inline-flex items-center justify-start gap-3 group">
                <input
                  type="checkbox"
                  class="ui-checkbox"
                  :class="selectAddress.defaultAddress ? 'cursor-no-drop' : 'cursor-pointer'"
                  v-model="selectAddress.defaultAddress"
                  :disabled="selectAddress.defaultAddress"
                />
                <span :class="{ 'opacity-50': selectAddress.defaultAddress }">Đặt làm địa chỉ mặc định</span>
                <div
                  class="absolute bottom-full left-0 mb-2 p-2 text-center bg-primaryColor text-whiteColor rounded-lg opacity-0 invisible transition-all duration-300 group-hover:visible group-hover:opacity-100"
                >
                  Bạn không thể thay đổi nhãn địa chỉ mặc định. Hãy đặt địa chỉ khác của bạn thành địa chỉ mặc định
                </div>
              </div>
            </div>
          </div>
          <div class="mt-6 flex items-center justify-end gap-4">
            <button
              @click.prevent="close"
              class="px-4 h-10 bg-dangerColor text-whiteColor transition-all duration-300 rounded-lg hover:bg-darkDangerColor cursor-pointer"
            >
              Hủy
            </button>
            <button @click.prevent="update" class="px-4 h-10 bg-primaryColor text-whiteColor transition-all duration-300 rounded-lg hover:bg-darkPrimaryColor">
              Cập nhật
            </button>
          </div>
        </form>
...
<script>
import { defineComponent, onMounted, reactive, ref, toRefs, computed, onUnmounted, watch } from 'vue'
import { useUserStore, useToastStore } from '@/stores'
import axios from 'axios'
import { updateAddress } from '@/webServices/userService'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const selectCity = (city, event) => {
      ...
      props.selectAddress.province = city.name
      props.selectAddress.district = ''
      props.selectAddress.ward = ''
    ...
    const selectDistrict = (district, event) => {
      ...
      props.selectAddress.district = district.name
      props.selectAddress.ward = ''
    ...
    const selectWard = (ward, event) => {
      ...
      props.selectAddress.ward = ward.name
      updateFullAddress()
      validateFullAddress()
      isSelectLocation.value = false
    }
    ...
    const updateFullAddress = () => {
      props.selectAddress.fullAddress = [props.selectAddress.province, props.selectAddress.district, props.selectAddress.ward].filter(item => item).join(', ')
    }
    ...
    const validateUsername = () => {
      ...
      if (!props.selectAddress.nameCustomer) {
    ...
    const validatePhoneNumber = () => {
      ...
      if (!props.selectAddress.phoneNumber) {
        errors.value.phoneNumber.push('Vui lòng nhập số điện thoại')
        isErrorPhoneNumber.value = true
      } else if (!/^[0-9]+$/.test(props.selectAddress.phoneNumber)) {
        errors.value.phoneNumber.push('Số điện thoại chỉ được phép chứa chữ số')
        isErrorPhoneNumber.value = true
      } else if (props.selectAddress.phoneNumber.length < 10 || props.selectAddress.phoneNumber.length > 15) {
    ...
    const validateSpecificAddress = () => {
      ...
      if (!props.selectAddress.specificAddress) {
    ...
    const update = async () => {
      ...
      const res = await updateAddress({
        id: props.selectAddress.id,
        nameCustomer: props.selectAddress.nameCustomer,
        phoneNumber: props.selectAddress.phoneNumber,
        specificAddress: props.selectAddress.specificAddress,
        ward: props.selectAddress.ward,
        district: props.selectAddress.district,
        province: props.selectAddress.province,
        fullAddress: props.selectAddress.fullAddress,
        defaultAddress: props.selectAddress.defaultAddress
      })
    ...
    const findSelectedCity = () => {
      const city = props.cities.find(city => city.name === props.selectAddress.province)
      if (city) {
        selectedCity.value = city.code
      }
    }
    ...
    const findSelectedDistrict = () => {
      const district = props.districts.find(district => district.name === props.selectAddress.district)
      if (district) {
        selectedDistrict.value = district.code
        filteredDistricts.value = props.districts.filter(district => district.province_code === selectedCity.value)
      }
    ...
    const findSelectedWard = () => {
      const ward = props.wards.find(ward => ward.name === props.selectAddress.ward)
      if (ward) {
        selectedWard.value = ward.code
        filteredWards.value = props.wards.filter(ward => ward.district_code === selectedDistrict.value)
      }
