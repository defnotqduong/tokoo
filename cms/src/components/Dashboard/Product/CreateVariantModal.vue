<template>
  <div>
    <input type="checkbox" id="my_modal_add_address" class="modal-toggle" v-model="isAddModalOpen" />
    <div class="modal" role="dialog">
      <div class="modal-box max-w-[800px] bg-white py-6 px-8">
        <div>
          <h4 class="text-xl font-bold">Thêm biến thể</h4>
        </div>
        <form class="form">
          <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12">
              <div class="input-group">
                ...
                <input type="text" name="name" id="name" placeholder="Tên biến thể" v-model="attribute" @input="validateAttribute" @blur="validateAttribute" />
              </div>
...
import { addVatirant } from '@/webServices/productService'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const add = async () => {
      ...
      formData.append('attribute', variant.attribute)
      formData.append('productId', props.productId)

      const res = await addVatirant(formData)
